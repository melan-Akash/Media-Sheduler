        ...
        if (generateImage) {
            try {
                ...
                if (hfToken && hfToken !== 'your_hugging_face_token_here') {
                    ...
                    const hfResponse = await axios.post('https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0', { inputs: imagePrompt }, {
...
export const schedulePost = async (req, res) => {
    try {
        ...
        const mappedPlatforms = (Array.isArray(parsedPlatforms) ? parsedPlatforms : []).map((p) => platformMapping[p.toLowerCase()] || p);
        let mediaUrl = req.body.mediaUrl;
        let mediaType = req.body.mediaType;
        if (req.file) {
            const result = await new Promise((resolve, reject) => {
                const stream = cloudinary.uploader.upload_stream({ resource_type: 'auto', folder: 'scheduler' }, (error, result) => {
                    if (error) {
                        reject(error);
                    }
                    else {
...
export const triggerCronPublish = async (req, res) => {
